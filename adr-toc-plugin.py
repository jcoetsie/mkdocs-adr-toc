...
class ADRTOCPlugin(BasePlugin):
    config_scheme = (
        ('toc_file', config.config_options.Type(str, default='index.md')),
        ('placeholder', config.config_options.Type(
            str, default='<!-- ADR_SUMMARY_PLACEHOLDER -->')),
    ...
    def on_page_markdown(self, markdown, page, config, files):
        ...
        if page.file.src_path == self.config.toc_file:
            logger.debug(
                f'Injecting ADR summary into page: {page.file.src_path}')

            # Define the directory containing the ADR records
            decisions_dir = os.path.join(
                config['docs_dir'], self.config.adr_path)
            ...
            logger.debug(
                f'Starting to process ADR files in directory: {decisions_dir}')
                    ...
                    with open(filepath, 'r') as file:
                        ...
                        if title_match:
                            ...
                            status = status_match.group(
                                1) if status_match else 'Unknown'
                            date = date_match.group(
                                1) if date_match else 'Unknown'
                            deciders = deciders_match.group(1).replace(
                                '"', '').strip() if deciders_match else 'Unknown'
                            adr_records.append({
                            ...
                            logger.warning(
                                f'Failed to extract title from file: {filepath}')
            ...
            existing_file = os.path.join(
                config['docs_dir'], self.config.adr_path, self.config.toc_file)
            if os.path.exists(existing_file):
                with open(existing_file, 'r') as file:
                    content = file.read()
                content = content.replace(
                    self.config.placeholder, adr_summary_content)
                with open(existing_file, 'w') as file:
                    file.write(content)
                logger.info(
                    f'Replaced {self.config.placeholder} in {existing_file}')
            else:
